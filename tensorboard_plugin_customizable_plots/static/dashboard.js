...
function smoothData(data, weight) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        smoothed.push({x: point.x, y: smoothedY});
...
const Runs = ({setIsLoading, checked, setChecked, runsConfig, setRunsConfig, deactivatedRuns, setDeactivatedRuns, filteredRuns}) => {
...
const RunsDrawer = ({setIsLoading, runsConfig, setRunsConfig, open, handleRunsDrawerClose, runs, deactivatedRuns, setDeactivatedRuns, runsDrawerWidth, setRunsDrawerWidth, filteredRuns, setFilteredRuns}) => {
    ...
    React.useEffect(()=>{
        // filtered
        let dr = filteredRuns.filter((run) => {return !checked[run]})
    ...
    const handleSearchChange = (event) => {
        ...
        let filtered_runs = runs.filter((e) => {return pattern.test(e)})
    ...
    return (
        ...
        >
            ...
            <Divider />
            <Runs setIsLoading={setIsLoading} checked={checked} setChecked={setChecked} runsConfig={runsConfig} setRunsConfig={setRunsConfig} runs={runs} deactivatedRuns={deactivatedRuns} setDeactivatedRuns={setDeactivatedRuns} filteredRuns={filteredRuns}/>
        </Drawer>
    );
}
...
function Dashboard({isLoading,
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            <RunsDrawer setIsLoading={setIsLoading} runsConfig={runsConfig} setRunsConfig={setRunsConfig} setRunsDrawerWidth={setRunsDrawerWidth} open={openRuns} handleRunsDrawerClose={handleRunsDrawerClose} runs={runs} deactivatedRuns={deactivatedRuns} setDeactivatedRuns={setDeactivatedRuns} runsDrawerWidth={runsDrawerWidth}  filteredRuns={filteredRuns}
        ...
        for(const tag in rawData) {
            ...
            for(const run in rawData[tag]){
                ...
                if(generalConfig.smoothing && generalConfig.smoothing > 0) {
                    ...
                    let dataPoints = trace.x.map((x, i) => ({x: x, y: trace.y[i]}));
